...
- **פרוקסי לניירות שאין ב-Yahoo (קרנות נאמנות ישראליות, מספר נייר 5xxxxxx)**:
  yfinance לא מכסה קרנות נאמנות פתוחות בת"א, אז המשתמש ממפה אותן לטיקר פרוקסי
  שעוקב אחרי אותו נכס בסיס (זהב→`GLD`, S&P500→`SPY`). הבעיה: הפרוקסי בדולר
  וההחזקה בש"ח — וערבוב היחידות נתן רווח/הפסד אבסורדי (+22562%) וסטופ שלילי.
  **הפתרון (החלטה):** הפרדת תפקידים בין שני מקורות הנתונים ב-`review_portfolio`:
  הפרוקסי מספק *רק* את הסיגנל הטכני (מגמה/RSI/MACD) ואת התנודתיות/רמות
  כ**אחוזים** (חסרי-מטבע, ראו `portfolio_bridge.proxy_levels`), בעוד שכל
  מספרי הכסף (רווח/הפסד, סטופ, יעד, מחיר נוכחי) מחושבים מההחזקה האמיתית בש"ח
  (`market_value/cost_basis/quantity`). `engine.review_holding` קיבל פרמטר
  `pnl_pct` אופציונלי כדי לקבל P&L אמיתי במקום לחשב מהשוואת מחיר-פרוקסי מול
  entry. החזקות כאלה מסומנות `is_proxy=True` וה-UI מציג הערה. הסטופ/יעד אינם
  משפיעים על ה-verdict ממילא (לא מאוחסנים, מוצעים מחדש בכל ריצה) — הם תצוגה
  בלבד. **לא הוספנו מקור נתונים ישראלי (TASE/maya)** — נשמר העיקרון "חינמי,
  yfinance בלבד"; פרוקסי + אחוזים נותן ניתוח טכני תקף בלי תלות חדשה.
- **מיפוי טיקרים (overrides) לפי מספר נייר, לא רק שם**: שם קרן נאמנות ת"א
  (כמו "מנוטרלת מט"ח - 500 P&S י(A4) הראל מחקה", מספר נייר 5127527) משתנה
  מעט בין ברוקרים/ייצואים (רווחים, ניקוד, פורמט), בעוד מספר הנייר קבוע —
  אז מיפוי שנשמר לפי שם בלבד (`load_symbol_overrides`/`resolve_symbol`
  הקודמים) לא תמיד תפס נייר זהה בייצוא הבא. **הפתרון:** `add_symbol_mapping`
  עדיין שומר גם name וגם asset_id בגיליון `symbol_overrides` (כך היה גם
  לפני), אבל `load_symbol_overrides` (כיום דרך `_overrides_from_df`, מופרד
  כדי להיבדק בלי תלות ב-gspread) מכניס כל מיפוי למילון **פעמיים** — גם לפי
  שם מנורמל וגם לפי asset_id מנורמל. ב-`symbols.resolve_symbol` סדר
  העדיפות: override-by-name → override-by-asset_id → טבלה סטטית לפי שם →
  asset_id אלפביתי (טיקר כבר-מוכן) → None. `sheets.gsheets.save_symbol_override`
  עודכן לעשות upsert גם לפי name וגם לפי asset_id (לא רק name), כדי שלא
  ייווצרו רשומות כפולות לאותו נייר תחת שמות שונים.

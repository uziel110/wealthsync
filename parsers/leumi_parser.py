"""
Parser for Bank Leumi / Leumi Trade (לאומי טרייד) securities portfolio Excel exports.

Expected export path:
  לאומי טרייד → תיק ניירות ערך → ייצוא ל-Excel
  OR
  הבנק הדיגיטלי → השקעות → ייצוא

Known column variants across Leumi portal versions:
  asset_name  : שם נייר ערך | שם הנייר | שם ני"ע | תיאור | שם נייר
  asset_id    : מספר נייר ערך | מספר נייר | מס' נייר | סמל
  quantity    : כמות | יחידות | כמות יחידות
  cost_basis  : עלות | עלות כוללת | עלות רכישה | שווי עלות | עלות ממוצעת כוללת
  market_value: שווי שוק | שווי שוק כולל | שווי נוכחי | ערך שוק | שווי
"""
...
_NAME_CANDIDATES  = ["שם נייר ערך", "שם הנייר", 'שם ני"ע', "תיאור", "שם נייר", "שם"]
_ID_CANDIDATES    = ["מספר נייר ערך", "מספר נייר", 'מס\' נייר', 'מספר ני"ע', "סמל", "קוד"]
_QTY_CANDIDATES   = ["כמות", "יחידות", "כמות יחידות", "כמות נייר"]
_COST_CANDIDATES  = ["עלות", "עלות כוללת", "עלות רכישה", "שווי עלות", "עלות ממוצעת כוללת"]
_MV_CANDIDATES    = ["שווי שוק", "שווי שוק כולל", "שווי נוכחי", "ערך שוק", "שווי"]
...
def _find_header_row(df_raw: pd.DataFrame) -> int:
    ...
    for i, row in df_raw.head(20).iterrows():
        ...
        if len(row_vals & all_candidates) >= 2:
            return int(i)
    raise ValueError(
        "לא זוהתה שורת כותרות מוכרת ב-20 השורות הראשונות של קובץ לאומי טרייד.\n"
        "ייצא את התיק מהאתר: תיק ניירות ערך ← ייצוא ל-Excel"
    )


def parse_leumi(file: "BytesIO | str", account_name: str = "לאומי טרייד") -> pd.DataFrame:
    """
    Parse a Bank Leumi / Leumi Trade securities Excel export.
